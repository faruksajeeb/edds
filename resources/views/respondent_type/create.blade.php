        ...
        <div class="col-md-8">
            <div class="card" style="background-color: #ECF4D6;">
                <div class="card-header bg-white">
                    <div class="row">
                        <div class="col-md-6">
                            <h3 class="card-title py-1"><i class="fa fa-plus"></i> Add Respondent Type</h3>
                        </div>
                        <div class="col-md-6">
                ...
                <div class="card-body">
                    <form action="{{ route('respondent_types.store') }}" method="POST" class="needs-validation" novalidate>

                        @csrf
                        <div class="form-group mb-3">
                            <label for="" class="@if ($errors->has('option')) has-error @endif fw-bold">Option <span class="text-danger">*</span></label><br />
                            <input name='option' id='option' class="form-control @error('option') is-invalid @enderror" placeholder="Enter option in english" value="{{ old('option') }}" required>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input name='label' id='label' class="form-control @error('label') is-invalid @enderror" placeholder="Enter label in english" value="{{ old('label') }}" required>
                            @if ($errors->has('label'))
                            @error('label')
                            <div class="alert alert-danger">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input name='label_bangla' id='label_bangla' class="form-control @error('label_bangla') is-invalid @enderror" placeholder="অনুগ্রহ করে বাংলায় লিখুন" value="{{ old('label_bangla') }}" required>
