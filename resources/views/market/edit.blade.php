    ...
    <div class="row">
        <div class="col-md-8">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('markets.update', Crypt::encryptString($marketInfo->id)) }}"
                        method="POST" class="needs-validation" novalidate>
                        @method('PUT')
                        @csrf
                        <div class="form-group my-1">
                            <label for="" class="@if ($errors->has('value_bangla')) has-error @endif fw-bold">Area *</label>
                            <select name="area_id" id="area_id" class="form-select" required>
                                <option value="">--select areas--</option>
                                @foreach ($areas as $val)                                    
                                    <option value="{{$val->id}}" {{ ($val->id==old('area_id',$marketInfo->area_id)) ?'selected':''}}>{{$val->value}}</option>
                                @endforeach
                            </select>
